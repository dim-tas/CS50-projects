...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char* val_ver = strstr(temp, "HTTP/1.1\r");
            if (val_ver == NULL)
            {
                error(505);  
            }
            ...
            char* temp3 = strchr(path, '.');
            char extension[strlen(temp3)+1];
            strcpy(extension, temp3+1);
...
const char* lookup(const char* extension)
{
    ...
    char* text_arr[3] = {"css", "html", "js"};
    char* img_arr[4] = {"gif", "ico", "jpg", "png"};
    char* text_resp[3] = {"text/css", "text/html", "text/javascript"};
    char* img_resp[4] = {"image/gif", "image/x-icon", "image/jpeg", "image/png"};
    for (int i = 0; i < 4; i++)
    {
        if (i < 3)
        {
            if (strcasecmp(text_arr[i], extension) == 0)
            {
                return text_resp[i];
            }
        }
        if (strcasecmp(img_arr[i], extension) == 0)
        {
            return img_resp[i];
        }
    }
